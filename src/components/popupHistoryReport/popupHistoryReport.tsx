...
export const PopupHistoryReport = React.forwardRef<HTMLDivElement, IPopupHistoryReportProps>(
  ({ closePopup, dataAboutReports }, ref) => {
    ...
    const liste: JSX.Element[] = dataAboutReports.reduce((accumulator, item) => {
      ...
      const li = (
        <li className={`text_type_main-medium ${styles.lineInList}`} key={uuid}>
          ...
          <ArrowRightIcon size="18" type="interface-primary" />
          <StatusLabel
    ...
    return (
      <Popup
        title="История отправки задания"
        closePopup={closePopup}
        externalClassName={styles.externalPopupClas}
        ref={ref}
      >
        <>
          <ul className={styles.list}>{liste}</ul>
          <Button
            htmlType="button"
            onClick={closePopup}
            size="small"
